Gsmooth = 10;  
file_location = 'n:\DATA\Betanitas\!Mouse Gramophon\2_Imaging\group33\33A2 (373, Dom)\33A2\Other\Analysis_backup\m33A_2.h5';
...
for iroi = 1:Ncells
    for istage = 1:Nstages
        ...
        for istim = 1:9
            path = strcat('/ANALYSIS/ROI_',num2str(iroi),'/STAGE_',num2str(istage),'/STIM_',num2str(istim));
            units = h5readatt(file_location,path,'UNITNUMBER');
            for irep = 1:length(units)
                path = strcat('/DATA/STAGE_',num2str(istage),'/UNIT_',num2str(units(irep)),'/ROI_',num2str(iroi),'/YDATA');
                raw(irep,:) = h5read(file_location,path);
            end
            for irep = 1:length(units)
                path = strcat('/DATA/STAGE_',num2str(istage),'/UNIT_',num2str(units(irep)),'/ROI_',num2str(iroi),'/BG');
                bg(irep,:) = h5read(file_location,path);
            end
            ...
            path = strcat('/ANALYSIS/ROI_',num2str(iroi),'/STAGE_',num2str(istage),'/STIM_',num2str(istim),'/DFF_CORR');
            try
                h5create(file_location,path,size(dff_new),'ChunkSize',size(dff_new),'Deflate', 9);
            catch
            end
            h5write(file_location, path, dff_new);
